...
func TestNewAdministratorFromDB(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			admin, err := NewAdministratorFromDB(uuid.New(), tc.firstName, tc.lastName, tc.email, tc.password, tc.gender, tc.birth, tc.phone, tc.lastLoginAt, tc.createdAt, tc.updatedAt, tc.deletedAt)
...
func TestNewAdministratorFromDB_Invalid(t *testing.T) {
	...
	admin, err := NewAdministratorFromDB(id, firstName, lastName, email, password, gender, birth, &phone, lastLoginAt, createdAt, updatedAt, nil)
