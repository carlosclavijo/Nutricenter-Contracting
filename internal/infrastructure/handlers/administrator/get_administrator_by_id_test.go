...
func TestAdministratorHandler_HandleGetById(t *testing.T) {
	...
	mockRepo.On("GetById", mock.Anything, mock.Anything).Return(admins[0], nil)

	cmd := queries.GetAdministratorByIdQuery{
		Id: admins[0].Id(),
	}
...
func TestHandleGetById_Error(t *testing.T) {
	...
	mockRepo.On("GetById", mock.Anything, mock.Anything).Return((*administrators.Administrator)(nil), ErrDbConnectionAdministrator)
