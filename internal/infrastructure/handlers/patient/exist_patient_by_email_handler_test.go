...
func TestPatientHandler_HandleExistByEmail(t *testing.T) {
	...
	cases := []struct {
		...
		{"repository error", false, false, ErrDbConnectionPatient, ErrDbConnectionPatient},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := queries.ExistPatientByEmailQuery{
				Email: email,
			}
