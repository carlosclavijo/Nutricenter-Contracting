...
func TestPatientHandler_HandleGetById(t *testing.T) {
	...
	mockRepo.On("GetById", mock.Anything, mock.Anything).Return(ptnts[0], nil)

	cmd := queries.GetPatientByIdQuery{
		Id: ptnts[0].Id(),
	}

	resp, err := handler.HandleGetById(ctx, cmd)

	assert.NoError(t, err)
	assert.NotNil(t, resp)
	assert.IsType(t, &dto.PatientDTO{}, resp)

	assert.Equal(t, ptnts[0].Id().String(), resp.Id)
	assert.Equal(t, ptnts[0].FirstName(), resp.FirstName)
	assert.Equal(t, ptnts[0].LastName(), resp.LastName)
	assert.Equal(t, ptnts[0].Email().Value(), resp.Email)
	assert.Equal(t, ptnts[0].Gender().String(), resp.Gender)
	assert.Equal(t, ptnts[0].Birth().Value(), resp.Birth)
...
func TestHandleGetById_Error(t *testing.T) {
	...
	mockRepo.On("GetById", mock.Anything, mock.Anything).Return((*patients.Patient)(nil), ErrDbConnectionPatient)
