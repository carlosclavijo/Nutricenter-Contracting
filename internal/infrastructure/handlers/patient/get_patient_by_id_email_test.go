...
func TestPatientHandler_HandleGetByEmail(t *testing.T) {
	...
	mockRepo.On("GetByEmail", mock.Anything, mock.Anything).Return(ptnts[0], nil)

	cmd := queries.GetPatientByEmailQuery{
		Email: ptnts[0].Email().Value(),
	}
...
func TestHandleGetByEmail_Error(t *testing.T) {
	...
	mockRepo.On("GetByEmail", mock.Anything, mock.Anything).Return((*patients.Patient)(nil), ErrDbConnectionPatient)
