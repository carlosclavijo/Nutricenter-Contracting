...
func (r *AdministratorRepository) Update(ctx context.Context, administrator *administrators.Administrator) (*administrators.Administrator, error) {
	//TODO implement me
	panic("implement me")
}
...
func (r *AdministratorRepository) Create(ctx context.Context, adm *administrators.Administrator) (*administrators.Administrator, error) {
	...
	query := `
		INSERT INTO administrators(id, first_name, last_name, email, password, gender, birth, phone)
...
func (r *AdministratorRepository) CountActive(ctx context.Context) (int, error) {
	var count int
	query := `SELECT COUNT(*) FROM administrators WHERE deleted_at IS NULL`
	err := r.Db.QueryRowContext(ctx, query).Scan(&count)
	if err != nil {
...
func (r *AdministratorRepository) CountDeleted(ctx context.Context) (int, error) {
	var count int
	query := `SELECT COUNT(*) FROM administrators WHERE deleted_at IS NOT NULL`
	err := r.Db.QueryRowContext(ctx, query).Scan(&count)
	if err != nil {
...
/*func (r *AdministratorRepository) Update(ctx context.Context, adm *administrators.Administrator) (*administrators.Administrator, error) {
	...
	query := `UPDATE administrators SET first_name = $1, last_name = $2, email = $3, password = $4, birth = $5, phone = $6, updated_at = DEFAULT WHERE id = $7 RETURNING id, first_name, last_name, email, birth, phone, last_login_at, created_at, updated_at, deleted_at`
	err := r.Db.QueryRowContext(ctx, query, adm.FirstName, adm.LastName, adm.Email, adm.Password, adm.Birth, adm.Phone).Scan(&idStr, &admin.FirstName, &admin.LastName, &admin.Email, &admin.Birth, &admin.Phone, &admin.LastLoginAt, &admin.CreatedAt, &admin.UpdatedAt, &admin.DeletedAt)

	if err != nil {
		log.Printf("infrastructure/persistence/repositories error executing SQL query in Update: %v", err)
		return nil, fmt.Errorf("scan failed: %w", err)
	}

	id, err := uuid.Parse(idStr)
	if err != nil {
		log.Printf("infrastructure/persistence/repositories error parsing Id '%s' in Update: %v", idStr, err)
	}
...
func (r *AdministratorRepository) Delete(ctx context.Context, id uuid.UUID) (*administrators.Administrator, error) {
	...
	query := `UPDATE administrators SET deleted_at = NOW() WHERE id = $1 RETURNING id, first_name, last_name, email, birth, phone, last_login_at, created_at, updated_at, deleted_at`
	err := r.Db.QueryRowContext(ctx, query, id).Scan(&idStr, &admin.FirstName, &admin.LastName, &admin.Email, &admin.Birth, &admin.Phone, &admin.LastLoginAt, &admin.CreatedAt, &admin.UpdatedAt, &admin.DeletedAt)

	if err != nil {
		log.Printf("infrastructure/persistence/repositories error executing SQL query in Delete: %v", err)
		return nil, err
	}

	idAdmin, err := uuid.Parse(idStr)
	if err != nil {
		log.Printf("infrastructure/persistence/repositories error parsing Id '%s' in Update: %v", idStr, err)
		return nil, err
	}
	admin.AggregateRoot = abstractions.NewAggregateRoot(idAdmin)
...
func (r *AdministratorRepository) Restore(ctx context.Context, id uuid.UUID) (*administrators.Administrator, error) {
	...
	query := `UPDATE administrators SET deleted_at = NULL WHERE id = $1 RETURNING id, first_name, last_name, email, birth, phone, last_login_at, created_at, updated_at, deleted_at`
	err := r.Db.QueryRowContext(ctx, query, id).Scan(&idStr, &admin.FirstName, &admin.LastName, &admin.Email, &admin.Birth, &admin.Phone, &admin.LastLoginAt, &admin.CreatedAt, &admin.UpdatedAt, &admin.DeletedAt)

	if err != nil {
		log.Printf("infrastructure/persistence/repositories error executing SQL query in Restore: %v", err)
		return nil, err
	}
	return &admin, nil
}*/
